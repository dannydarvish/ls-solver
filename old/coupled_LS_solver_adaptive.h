...
public:
    TransitionMatrixSolver(double kp, int beta, vector<vector<cdouble (*)(double)> > g,
        ...
        double ep, int max_dim = 10000):
            kp(kp), beta(beta), g(g), v(v), m_sigma(m_sigma), m_alpha(m_alpha),
            k_max(k_max), dk_tol(dk_tol), n_subint_steps(3), ep(ep),
            ...
            max_dim(max_dim)
    {
        ...
        // Doing so allows us to speed up the quadrature by integrating out the weight function,
        // w_gamma(k) = 1/(E-2sqrt(m^2+k^2)) analytically
        for (const auto & chan : m_alpha)
            assert(chan.first == chan.second);
        // // For now, we need four integration points per subinterval.
